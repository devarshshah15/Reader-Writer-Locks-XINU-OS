...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pargs = 0;
	pptr->pprio = 0;
	pptr->pinh=0;  /* inherited priority for priority inheritance protocol */
	pptr->plockid=-1; /* when the process is not blocked */
	currpid = NULLPROC;
    i=0;
	while(i<NLOCKS)
	{
		pptr->plocksheld[i]=0; /*initially no locks will be held by the process when it is created*/
		pptr->plocksused[i]= 0;
		i++;
	}
